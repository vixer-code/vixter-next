...
const CreatePackModal: React.FC<CreatePackModalProps> = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    features: [] as string[],
    tags: [] as string[],
    licenseOptions: [] as string[], // ['personal','commercial','editorial','resale'] when packType == 'nao-download'
    coverImage: null as string | null, // URL when editing
    sampleImages: [] as string[], // existing URLs when editing
    sampleVideos: [] as string[], // existing URLs when editing
    packContent: [] as string[] // existing URLs when editing
  })
  ...
  const [sampleVideoPreviews, setSampleVideoPreviews] = useState<string[]>([])
  const [packFiles, setPackFiles] = useState<File[]>([]) // images or videos (final downloadable content)
  const [packFilePreviews, setPackFilePreviews] = useState<Array<{src: string, isVideo: boolean}>>([]) // data URLs for previews aligned with packFiles
  ...
  const handleSampleImagesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (total > MAX_SAMPLES) {
      alert(`Máximo de ${MAX_SAMPLES} amostras (imagens ou vídeos)`)
      return
    }
    setSampleImageFiles(prev => [...prev, ...files])
    files.forEach(file => {
      const reader = new FileReader()
      reader.onload = ev => setSampleImagePreviews(prev => [...prev, ev.target?.result as string])
      reader.readAsDataURL(file)
    })
  }

  const handleSampleVideosChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || [])
    const total = sampleImageFiles.length + sampleVideoFiles.length + files.length
    if (total > MAX_SAMPLES) {
      alert(`Máximo de ${MAX_SAMPLES} amostras (imagens ou vídeos)`)
    ...
    files.forEach(file => {
      const reader = new FileReader()
      reader.onload = ev => setSampleVideoPreviews(prev => [...prev, ev.target?.result as string])
      reader.readAsDataURL(file)
    })
  }
  ...
  const removeSampleVideo = (index: number) => {
  ...
  const handlePackFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    files.forEach(file => {
      const reader = new FileReader()
      reader.onload = ev => {
        setPackFilePreviews(prev => [...prev, { src: ev.target?.result as string, isVideo: file.type.startsWith('video/') }])
      }
      reader.readAsDataURL(file)
    })
  }
  ...
  const removeExistingPackContent = (index: number) => {
    setFormData(prev => ({
      ...prev,
  ...
  const validateCurrentStep = () => {
    switch (currentStep) {
      case 0: // basic
        ...
        if (subcategoriesMap[formData.category as keyof typeof subcategoriesMap]?.length && !formData.subcategory) return false
  ...
  const convertVCtoVP = (vcAmount: number) => vcAmount * 1.5
  const formatCurrency = (amount: number, decimals = 2) => {
    return parseFloat(amount.toString()).toFixed(decimals).replace('.', ',')
  }
  const formatVC = (amount: number) => `${formatCurrency(amount)} VC`
  const formatVP = (amount: number) => `${formatCurrency(amount)} VP`
  ...
  const handleSubmit = async () => {
    ...
    if (subcategoriesMap[formData.category as keyof typeof subcategoriesMap]?.length && !formData.subcategory) {
      alert('Por favor, selecione uma subcategoria')
      return
    }
    if (!formData.packType) {
      alert('Por favor, selecione um tipo de pack')
      return
    }
    ...
    if (!coverImageFile && !formData.coverImage) {
      alert('Você deve carregar uma imagem de capa')
      return
    }
    ...
    try {
      ...
      const packData = {
        ...
        tags: formData.tags,
        licenseOptions: formData.packType === 'nao-download' ? formData.licenseOptions : [],
        status: 'active'
      }
      ...
      alert(`Falha ao ${editingPack ? 'atualizar' : 'criar'} pack`)
    } finally {
  ...
  const categoryLabel = (val: string) => packCategories.find(c => c.value === val)?.label || val
  const subcategoryLabel = (cat: string, subVal: string) => {
    const match = subcategoriesMap[cat as keyof typeof subcategoriesMap]?.find(s => s.toLowerCase().replace(/\s+/g, '-') === subVal)
    return match || subVal || ''
  }
        ...
        <div className="modal-content">
          {currentStep === 0 && (
            <div className="form-step">
              ...
              {formData.category && subcategoriesMap[formData.category as keyof typeof subcategoriesMap] && (
                <div id="subcategory-container" className="form-group">
                  ...
                  >
                    <option value="" />
                    {subcategoriesMap[formData.category as keyof typeof subcategoriesMap].map(sub => {
                    ...
                    {formData.discount && parseInt(formData.discount, 10) > 0 ? (
                      <>
                        <div className="preview-price-secondary">
                          {formatVC(parseFloat(formData.price))}
                        </div>
                      ...
                      <div className="preview-price-main">
                        {formData.price ? formatVP(convertVCtoVP(parseFloat(formData.price))) : '0,00 VP'}
                      </div>
                    )}
                  </div>
                </div>
