...
const CreateServiceModal: React.FC<CreateServiceModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    price: '',
    features: [] as string[],
    complementaryOptions: [] as any[],
    coverImage: null as string | null,
    showcasePhotos: [] as string[],
    showcaseVideos: [] as string[],
    tags: [] as string[]
  })
  ...
  const getPriceSuggestions = (category: string) => {
    const suggestions = {
      games: [
  ...
  const autoSave = async () => {
    ...
    try {
      ...
      localStorage.setItem(`service-draft-${session.user.id}`, JSON.stringify(draftData))
  ...
  const loadDraft = () => {
    ...
    try {
      const draft = localStorage.getItem(`service-draft-${session.user.id}`)
      if (draft) {
  ...
  const populateFormWithServiceData = (service: any) => {
    setFormData({
      title: service.title || '',
  ...
  const updateComplementaryOption = (index: number, field: string, value: string) => {
    setFormData(prev => ({
      ...prev,
  ...
  const removeComplementaryOption = (index: number) => {
    setFormData(prev => ({
      ...prev,
  ...
  const handleShowcasePhotosChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length + showcasePhotoFiles.length <= 10) {
      ...
      files.forEach(file => {
        const reader = new FileReader()
        reader.onload = (e) => {
          setShowcasePhotoPreviews(prev => [...prev, e.target?.result as string])
        }
        reader.readAsDataURL(file)
      })
    }
  ...
  const handleShowcaseVideosChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(event.target.files || [])
    if (files.length + showcaseVideoFiles.length <= 5) {
      ...
      files.forEach(file => {
        const reader = new FileReader()
        reader.onload = (e) => {
          setShowcaseVideoPreviews(prev => [...prev, e.target?.result as string])
        }
        reader.readAsDataURL(file)
      })
    }
  ...
  const getOptionPriceValidationError = (price: string) => {
    if (!price) {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.price || parseFloat(formData.price) < 10) {
      alert('O preço mínimo é 10,00 VC')
      return
    }

    // Check if cover image is provided
    if (!coverImageFile && !formData.coverImage) {
      alert('Você deve carregar uma imagem de capa para o serviço')
      return
    }
    ...
    for (let i = 0; i < formData.complementaryOptions.length; i++) {
      ...
      if (!option.title.trim()) {
        alert(`Por favor, insira um título para a opção complementar ${i + 1}`)
        return
      }
      if (!option.description.trim()) {
        alert(`Por favor, insira uma descrição para a opção complementar ${i + 1}`)
        return
      }
      if (!option.price || parseFloat(option.price) < 1) {
        alert(`O preço mínimo para a opção complementar ${i + 1} é 1,00 VC`)
        return
      }
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.removeItem(`service-draft-${session.user.id}`)
      ...
      alert(`Falha ao ${editingService ? 'atualizar' : 'criar'} serviço`)
    } finally {
  ...
  const convertVCtoVP = (vcAmount: number) => {
    return vcAmount * 1.5
  }
  ...
  const formatVC = (amount: number) => {
    return `${formatCurrency(amount)} VC`
  }

  const formatVP = (amount: number) => {
    return `${formatCurrency(amount)} VP`
  }

  const handlePriceSuggestion = (suggestion: any) => {
              ...
              <div className="form-group">
                ...
                {showPriceSuggestions && formData.category && (
                  <div className="price-suggestions">
                    <h4>Sugestões de Preço para {categoryLabels[formData.category as keyof typeof categoryLabels]}</h4>
                  ...
                  <div className="currency-display">
                    ...
                    <div className="currency-row">
                      ...
                      <span className="currency-value vp">
                        {formatVP(convertVCtoVP(parseFloat(formData.price)))}
                      </span>
                    </div>
                        ...
                        {option.price && !optionPriceError && (
                          <div className="currency-display option-currency-display">
                            <div className="currency-row">
                              <span>Você recebe:</span>
                              <span className="currency-value vc">
                                {formatVC(parseFloat(option.price))}
                              </span>
                            </div>
                            <div className="currency-row">
                              <span>Cliente paga:</span>
                              <span className="currency-value vp">
                                {formatVP(convertVCtoVP(parseFloat(option.price)))}
                              </span>
                            </div>
                          </div>
                        )}
                  ...
                  <div className="preview-service-info">
                    ...
                    <div className="preview-meta">
                      <span className="preview-category">
                        {categoryLabels[formData.category as keyof typeof categoryLabels] || 'Categoria'}
                      </span>
                    </div>
                  </div>
                  <div className="preview-price-container">
                    <div className="preview-price-main">
                      A partir de {formData.price ? formatVP(convertVCtoVP(parseFloat(formData.price))) : '0,00 VP'}
                    </div>
                  </div>
                </div>
                    ...
                    {formData.complementaryOptions.map((option, index) => (
                      <div key={index} className="preview-option">
                        ...
                        <div className="option-pricing">
                          <div className="option-price-main">+{formatVP(convertVCtoVP(parseFloat(option.price)))}</div>
                          <div className="option-price-secondary">Você recebe: +{formatVC(parseFloat(option.price))}</div>
                        </div>
                      </div>
                    ))}
                  </div>
